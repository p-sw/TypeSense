...
def update_words(level: str):
    ...
    while not url_end:
        ...
        try:
            ...
            logger.crit('TIMEOUT ON THREAD '+level)
            url_end = True
        if url_end is False:
            ...
            file.write(curr_img)
            file.close()
            logger.info("Download finished: " + curr_filename)
            logger.info("Word Detecting..")
            for file in dir_list:
                ...
                if img_dt == curr_img:
                    ...
                    selfdict[url] = cw
                    logger.info('Word detected: {}'.format(cw))
        url_num += 1
