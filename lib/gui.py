...
from lib.Gui.combobox import ComboBox
...
class GUI:
    def __init__(self):
        ...
        self.SelectionTest = ComboBox(10, 180, 200, ["One", "Two", "Three"], "SelectionTest", 15)
        ...
        self.combo_sprite_group = pygame.sprite.Group()
        ...
        self.combo_sprite_group.add(self.SelectionTest)
    ...
    def run(self):
        while not self.stopped:
            ...
            self.combo_sprite_group.update()
            for s_obj in self.interactive_sprite_group:
                s_obj.check(vnt)
            for s_obj in self.combo_sprite_group:
                s_obj.check(vnt)

            self.interactive_sprite_group.draw(self.display)
            self.combo_sprite_group.draw(self.display)

            for s_obj in self.interactive_sprite_group:
                s_obj.font_draw(self.display)
            for s_obj in self.combo_sprite_group:
                s_obj.font_draw(self.display)
                s_obj.activated_draw(vnt, self.display)
